...
::: {.callout-warning}
## Prerequisites
**Required**: [Article 5: Features](05-features.qmd) — understanding what features are and why neurons are polysemantic
:::

::: {.callout-note}
## Before You Read: Recall
From Article 5, recall:
...
//| echo: false
md`**Analysis:**
- **Features**: ${numFeatures} packed into ${dimensions} dimensions
- **Angle between adjacent features**: ${(360 / numFeatures).toFixed(1)}° ${360/numFeatures >= 90 ? "(orthogonal)" : 360/numFeatures >= 60 ? "(good separation)" : "(crowded)"}
- **Geometric interference** (cosine similarity): ${(worstInterference * 100).toFixed(1)}%
- **Co-occurrence probability** (at ${(sparsity*100).toFixed(0)}% activation): ${(cooccurrence * 100).toFixed(2)}%
- **Expected total interference**: ${effectiveInterference < 0.05 ? "✓ Low — superposition works well" : effectiveInterference < 0.15 ? "⚠ Moderate — acceptable trade-off" : "✗ High — consider fewer features"}

*The key insight: even with high geometric interference, low activation probability keeps expected interference manageable.*`
```
...
The transition is *sudden*, not gradual—small changes in sparsity cause large changes in representation structure. This matters because:
