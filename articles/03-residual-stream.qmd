...
```{mermaid}
%%| fig-cap: "Components read from and write to a shared residual stream. Information flows through this common workspace."
flowchart LR
    subgraph "Residual Stream"
        direction TB
        X0["x₀ (embedding)"]
        X1["x₁ = x₀ + contributions"]
        X2["x₂ = x₁ + contributions"]
        XL["xₗ (final)"]
        X0 --> X1 --> X2 --> XL
    end

    H1["Attn Head 1.0"] -.->|"read"| X0
    H1 -.->|"write"| X1
    H2["Attn Head 1.1"] -.->|"read"| X0
    H2 -.->|"write"| X1
    M1["MLP Layer 1"] -.->|"read"| X1
    M1 -.->|"write"| X2
    H3["Attn Head 2.0"] -.->|"read"| X1
    H3 -.->|"write"| X2
```
...
Consider the famous **induction head** circuit. An induction head performs in-context learning: if it sees "...Harry Potter... Harry" it predicts "Potter" because it saw that pattern before.
