...
We've spent the last three articles building up the atomic theory of neural network representations:

- **Article 5**: Features are directions in activation space
- **Article 6**: Superposition packs many features into few dimensions
- **Article 7**: Toy models show this is the optimal compression strategy
...
Individual neurons (or more precisely, directions in activation space) learn to detect specific, interpretable patterns. We established this in Articles 5-7.
...
**3. Induction Heads**: Use patterns to identify the indirect object position
...
```{mermaid}
%%| fig-cap: "The IOI circuit: 7 functional groups of attention heads work together to identify and output the indirect object."
flowchart TD
    subgraph "Early Layers (1-2)"
        PT[Previous Token Heads]
        DT[Duplicate Token Heads]
    end

    subgraph "Middle Layers (3-6)"
        IND[Induction Heads]
        SI[S-Inhibition Heads]
    end

    subgraph "Late Layers (7-10)"
        NM[Name Mover Heads]
        NNM[Negative Name Movers]
        BNM[Backup Name Movers]
    end

    PT -->|"position info"| IND
    DT -->|"subject marked"| SI
    IND -->|"IO position"| NM
    SI -->|"suppress subject"| NM
    NM -->|"boost 'Mary'"| Output[Predict: Mary]
    NNM -->|"suppress 'John'"| Output
    BNM -.->|"backup path"| Output
```
...
All three work in parallel. The final prediction is the sum of their contributions (remember the residual stream from Article 3).
...
One of the most important circuits in transformers. We'll cover these in depth in Article 13, but the basic idea:
...
Remember Article 5: most neurons are polysemantic, responding to multiple unrelated features. This means:
...
From Article 6: features are superimposed, not cleanly separated. This creates problems:
...
This article applies Polya's heuristic: **find a related problem you understand**.
...
The next arc (Articles 9-12) covers these techniques:
- **Sparse Autoencoders** (Article 9): The tool for extracting features from superposition
- **Attribution** (Article 10): Tracing which components contribute to outputs
- **Activation Patching** (Article 11): Causal verification through intervention
- **Ablation** (Article 12): Understanding what happens when you remove components
...
4. **In-Context Learning and Induction Heads** — [Anthropic](https://transformer-circuits.pub/2022/in-context-learning-and-induction-heads/index.html): How induction circuits enable in-context learning (preview of Article 13).
