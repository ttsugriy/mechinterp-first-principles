...
In Article 10, we learned to ask: "What contributed to this output?" Attribution decomposes the logit into per-component contributions, showing which attention heads and MLPs pushed the prediction in which direction.
...
For the IOI task from Article 8:
...
```{mermaid}
%%| fig-cap: "Activation patching: run clean and corrupted inputs, then patch one component's activation from clean into corrupted to measure causal effect."
flowchart TB
    subgraph "Clean Run"
        CI[Clean Input] --> CM1[Layer 1] --> CM2[Layer 2] --> CM3[Layer 3] --> CO["Output: Mary ✓"]
        style CM2 fill:#90EE90
    end

    subgraph "Corrupted Run"
        XI[Corrupted Input] --> XM1[Layer 1] --> XM2[Layer 2] --> XM3[Layer 3] --> XO["Output: John ✗"]
        style XM2 fill:#FFB6C1
    end

    subgraph "Patched Run"
        PI[Corrupted Input] --> PM1[Layer 1] --> PM2["Layer 2<br/>(PATCHED)"] --> PM3[Layer 3] --> PO["Output: Mary? ✓"]
        style PM2 fill:#90EE90
    end

    CM2 -.->|"copy activation"| PM2
```
...
Using sparse autoencoders (Article 9), we can patch individual features:
...
The IOI circuit (Article 8) was discovered using exactly this methodology:
...
But there's a related question: "What happens if we completely remove this component?" This is **ablation**—the subject of the next article.
