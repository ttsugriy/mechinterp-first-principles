...
def get_top_predictions(model, prompt, k=5):
    ...
    for logit, token in zip(top_logits, top_tokens):
        ...
        prob = torch.softmax(logits[0, -1], dim=-1)[token].item()
        predictions.append((word, prob, logit.item()))
    return predictions
...
def get_token_logit(model, prompt, target_word):
...
```python
def logit_attribution(model, prompt, target_token):
    """
    Compute each component's contribution to the target token's logit.
    Returns a dict of component -> contribution.
    """
    ...
    # Get the unembedding direction for our target
    target_dir = model.W_U[:, target_id]  # Shape: [d_model]

    contributions = {}

    # Embedding contribution
    embed = cache["embed"][0, -1]  # Last position
    contributions["embed"] = (embed @ target_dir).item()

    # Position embedding contribution
    pos_embed = cache["pos_embed"][0, -1]
    contributions["pos_embed"] = (pos_embed @ target_dir).item()

    # Each layer's attention and MLP
    for layer in range(model.cfg.n_layers):
        # Attention heads (summed)
        attn_out = cache["attn_out", layer][0, -1]
        contributions[f"L{layer}_attn"] = (attn_out @ target_dir).item()
