...
def logit_attribution(model, prompt: str, target_token: str) -> tuple[dict[str, float], float]:
    ...
    target_dir = model.W_U[:, target_id]  # <2>

    # Start with embedding contributions
    contributions = {
        "embed": (cache["embed"][0, -1] @ target_dir).item(),  # <3>
        "pos_embed": (cache["pos_embed"][0, -1] @ target_dir).item(),
    }

    # Add each layer's attention and MLP contributions
...
1. `run_with_cache` stores all intermediate activations—this is what makes TransformerLens powerful for interpretability.
2. The unembedding matrix column for our target token. This is the "direction" in residual stream space that, when projected onto, gives the logit for this token.
3. The dot product measures how much this component's output points toward predicting our target token. Positive = helps predict it, negative = suppresses it.
...
def patch_position(model, clean_prompt, corrupted_prompt, position, layer):
    ...
    from corrupted into clean.
    """
    corrupted_tokens = model.to_tokens(corrupted_prompt)
    _, corrupted_cache = model.run_with_cache(corrupted_tokens)
    corrupted_resid = corrupted_cache["resid_post", layer]  # <1>

    def patch_hook(resid, hook):  # <2>
        resid[:, position, :] = corrupted_resid[:, position, :]
        return resid

    clean_tokens = model.to_tokens(clean_prompt)
    patched_logits = model.run_with_hooks(  # <3>
        clean_tokens,
...
1. Get the residual stream *after* this layer processes. This contains all information the model has built up to this point.
2. A hook is a function that intercepts and optionally modifies activations during the forward pass. Here we surgically replace just one position.
3. `run_with_hooks` executes the forward pass but calls our hook at the specified location. This is the causal intervention—we're changing what the model sees.
